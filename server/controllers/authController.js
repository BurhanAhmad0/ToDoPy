...
const registerUser = async (req, res) => {
  try {
    ...
    <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
      ...
      <p>Dear User,</p>
      <p>We’re excited to welcome you to Codiqon! Your account has been created successfully.</p>
      <p>If you have any questions or need assistance, feel free to reach out to our support team.</p>
      <p>Best regards,<br>The Codiqon Team</p>
    </div>
  `,
    };

    // Send welcome email
    const info = await nodemailerConfig.sendMail(emailOptions);
...
const loginUser = async (req, res) => {
  try {
    ...
    res.cookie("SESSION_TOKEN", token, {
      ...
      sameSite: "none", // Adjust sameSite policy based on environment
...
const user = async (req, res) => {
  try {
    ...
    const user = await UserModel.findById(decoded.userId).select(
      "-password -lists"
    ); // Exclude password and lists from response
