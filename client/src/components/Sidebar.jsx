...
const Sidebar = ({
  ...
  const handleSidebarExpansion = () => {
    try {
      // Compute the new sidebar state
      const newSidebarState = !ExpandSidebar;

      // Update the state
      setExpandSidebar(newSidebarState);

      // Persist the new state to localStorage
      localStorage.setItem("sidebarExpansion", JSON.stringify(newSidebarState));
    } catch (error) {
      console.error("Error handling sidebar expansion:", error);
    }
  };
  ...
  useEffect(() => {
    ...
    const checkSidebarExpanded = () => {
      const sidebarExpanded = localStorage.getItem("sidebarExpansion");

      if (sidebarExpanded !== null) {
        setExpandSidebar(JSON.parse(sidebarExpanded)); // Convert string to boolean
      } else {
        setExpandSidebar(false); // Default fallback
      }
    };
    ...
    <>
      <aside
        className={`hidden lg:block lg:${
          ExpandSidebar ? "w-1/4" : "w-28"
        } rounded-4xl bg-sidebar-background dark:bg-green-900 p-8 transition-all duration-300 text-black dark:text-white`}
      >
        ...
        <div
          className={`search relative ${ExpandSidebar ? "block" : "hidden"}`}
        >
          <input
        ...
        <div className="tasks mt-7 h-56">
          ...
          {ExpandSidebar && (
            <>
              <Link
                to={`/${user?.firstName.toLowerCase()}/upcoming`}
                className={`flex gap-2 mt-3 cursor-pointer transition-all duration-300 ${
                  location.pathname.includes("/upcoming") &&
                  "bg-black/10 dark:bg-white/10 p-2 rounded-lg"
                }`}
              >
                <img
              ...
              <Link
                to={`/${user?.firstName.toLowerCase()}/tasks`}
                className={`flex gap-2 mt-3 cursor-pointer transition-all duration-300 ${
              ...
              <Link
                to={`/${user?.firstName.toLowerCase()}/calender`}
                className={`flex gap-2 mt-3 cursor-pointer transition-all duration-300 ${
              ...
              <Link
                to={`/${user?.firstName.toLowerCase()}/notes`}
                className={`flex gap-2 mt-3 cursor-pointer transition-all duration-300 ${
          ...
          <div className="listsCards h-36 overflow-y-auto">
            {ExpandSidebar ? (
              !Lists.length ? (
                ...
                Lists.map((list) => (
                  <Link
                    to={`/${user?.firstName.toLowerCase()}/list/${list._id}`}
                    key={list._id}
      ...
      <aside
        className={`block lg:hidden w-full xsm:w-1/2 ${
          IsModbileSideberOpen ? "translate-x-0" : "-translate-x-full"
        } h-full sm:h-screen absolute top-0 left-0 z-10 bg-sidebar-background dark:bg-green-900 text-black dark:text-white p-3 transition-all duration-300`}
      >
        {/* Header */}
        ...
        <div className="tasks mt-7">
          ...
          <Link
            to={`/${user?.firstName.toLowerCase()}/upcoming`}
            className={`flex gap-2 mt-3 items-center transition-all duration-300 ${
              location.pathname === `/${user?.firstName.toLowerCase()}/upcoming`
                ? "bg-black/10 dark:bg-white/10 p-2 rounded-lg"
                : ""
            }`}
          >
          ...
          <Link
            to={`/${user?.firstName.toLowerCase()}/tasks`}
            className={`flex gap-2 mt-3 items-center transition-all duration-300 ${
              location.pathname === `/${user?.firstName.toLowerCase()}/tasks`
                ? "bg-black/10 dark:bg-white/10 p-2 rounded-lg"
                : ""
            }`}
          >
          ...
          <Link
            to={`/${user?.firstName.toLowerCase()}/calender`}
            className={`flex gap-2 mt-3 items-center transition-all duration-300 ${
              location.pathname === `/${user?.firstName.toLowerCase()}/calender`
                ? "bg-black/10 dark:bg-white/10 p-2 rounded-lg"
                : ""
            }`}
          >
          ...
          <Link
            to={`/${user?.firstName.toLowerCase()}/notes`}
            className={`flex gap-2 mt-3 items-center transition-all duration-300 ${
              location.pathname === `/${user?.firstName.toLowerCase()}/notes`
                ? "bg-black/10 dark:bg-white/10 p-2 rounded-lg"
                : ""
            }`}
          >
        ...
        <div className="lists mt-7">
          ...
          <button
            title="Create List"
            onClick={() => setCreateListModal(true)}
            className="flex gap-2 mt-3 items-center text-left"
          >
            <img
              className="filter dark:invert"
              loading="lazy"
